#!/usr/bin/env python3
# ============================================================
#  모양 인식 - 3D 프린트용 STL 생성 (실물 테스트용 3종 도형)   [부록·준비물]
#   구 · 타원(럭비공) · 정육면체
#   순수 파이썬 기하 계산으로 STL(삼각면 메시) 파일을 씀. MuJoCo·하드웨어 불필요.
#
...
#     · 7절 = 단위 SI(길이 m). 시뮬은 m, 여기 프린트는 mm → 같은 실물 치수여야 sim2real 성립.
#     · 실물 물체 = 시뮬의 정적 body 물체와 동일 치수 → 손가락 깊이 패턴이 일치(9절).
#     · 이 도형들을 STEP3·5·6 에서 실제로/가상으로 잡아 모양을 인식.
#
#  실행:  python3 day2_부록_shape_print_stl.py   →  stl/구.stl …
#  준비:  pip install numpy
