...
#
#  ┌ 잘 안 될 때 어디를 고치나 ─────────────────────────────┐
#  │ 물체 쥐었는데 "안 잡음"   → MARGIN 낮추기(10→8) 또는      │
#  │                            CLOSE_TARGET 높이기(120→140)  │
#  │ 빈손인데 "잡음"으로 뜸    → MARGIN 높이기(10→14)          │
...
DEFAULT = {
    "OPEN_BEND":    -80,     # 펴기
    "CLOSE_TARGET": 120,     # 쥐는 힘/깊이 — 낮출수록 약하게 쥠(하드웨어 보호), 높일수록 확실히 쥠
    "GAP":          12,      # 감지 예민도(캘리브 없을 때 폴백) — 낮출수록 예민
    "MARGIN":       10,      # 캘리브 기준보다 이만큼 덜 닫히면 '잡음' — 낮출수록 예민
